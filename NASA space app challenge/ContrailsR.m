...
input_path = strcat('res/',filename);
outout_path = strcat('rst/',filename);
...
new_image = medfilt2(new_image,[3,3]);
...
lines = houghlines(new_image,T,R,P,'FillGap',200,'MinLength',50);
figure(result), imshow(I), hold on
...
title('Final output');
saveas(figure(result), fullfile(output_path), 'jpeg');
